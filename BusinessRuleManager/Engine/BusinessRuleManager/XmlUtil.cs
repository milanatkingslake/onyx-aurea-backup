...
namespace ConsonaCRM.BusinessRuleManager
{
	...
	public static class XmlUtil
	{
		...
		/// <param name="pParentNode">Parent reference node for XPath expression.</param>
		/// <param name="psXPath">XPath expression to evaluate.</param>
		/// <param name="pNamespaceManager">XML namespace manager for the document.</param>
		/// <returns>XMLNode referencing required node.</returns>
		public static XmlNode GetRequiredNode(XmlNode pParentNode, string psXPath, XmlNamespaceManager pNamespaceManager)
		{
			XmlNode childNode = pParentNode.SelectSingleNode(psXPath, pNamespaceManager);
			if (childNode == null)
			{
				string assemblyClassName = MethodBase.GetCurrentMethod().DeclaringType.Name;
				throw new BusinessRuleManagerException(assemblyClassName,
		...
		/// <param name="pNamespaceManager">XML namespace manager for the document.</param>
		/// <returns>String value of the node.</returns>
		public static string GetRequiredNodeValue(XmlNode pParentNode, string psXPath, XmlNamespaceManager pNamespaceManager)
		{
			return GetRequiredNode(pParentNode, psXPath, pNamespaceManager).InnerText;
		}
		/// <summary>
		...
		public static string GetNodeValue(XmlNode pParentNode, string psXPath, bool pbHandleNull, string psDefaultValue)
		{
			...
			if (childNode.NodeType == XmlNodeType.Element)
			{
				if (pbHandleNull)
				{
					XmlAttribute nullAttr = (XmlAttribute)childNode.Attributes.GetNamedItem(BusinessRuleManagerAttributeName.Null);
					if (nullAttr != null)
					{
		...
		/// <param name="psDefaultValue">Default value to return if the node is not found.</param>
		/// <returns>Value of the node.</returns>
		public static string GetNodeValue(XmlNode pParentNode, string psXPath,
			XmlNamespaceManager pNamespaceManager, bool pbHandleNull, string psDefaultValue)
		{
			XmlNode childNode = pParentNode.SelectSingleNode(psXPath, pNamespaceManager);
			if (childNode == null)
			{
		...
		/// <param name="pNamespaceManager">XML namespace manager for the document.</param>
		/// <returns>Child node.</returns>
		public static XmlNode GetOrCreateNode(XmlNode pParentNode, string psXPath, XmlNamespaceManager pNamespaceManager)
		{
			// Find last slash character
			int separatorIndex = psXPath.LastIndexOf("/");
			// If the xpath has a root slash, then we're looking for a named root node
			...
			if (separatorIndex == 0)
			{
				return GetRequiredNode(pParentNode, psXPath, pNamespaceManager);
			}
			...
			if (childNode == null)
			{
				...
				if (separatorIndex > 0)
				{
					pParentNode = GetOrCreateNode(pParentNode, psXPath.Remove(separatorIndex), pNamespaceManager);
				}
				// Add child node (element or attribute)
				string nodeName = psXPath.Substring(separatorIndex + 1);
				if (nodeName.StartsWith("@"))
				...
				{
					childNode = pParentNode.AppendChild(pParentNode.OwnerDocument.CreateElement(psXPath.Substring(separatorIndex + 1), pNamespaceManager.DefaultNamespace));
				}
			}
		...
		public static XmlNode AddChildNode(XmlNode pParentNode, string psNodeName)
		{
			...
			{
				XmlAttribute newAttr = pParentNode.OwnerDocument.CreateAttribute(psNodeName.Substring(1));
				pParentNode.Attributes.SetNamedItem(newAttr);
				return newAttr;
			}
			else
			{
		...
		/// <param name="pNamespaceManager">XML namespace manager for the document.</param>
		/// <returns>XmlNode for new child element.</returns>
		public static XmlNode AddChildNode(XmlNode pParentNode, string psNodeName, XmlNamespaceManager pNamespaceManager)
		{
			if (psNodeName.StartsWith("@"))
			{
				XmlAttribute newAttr = pParentNode.OwnerDocument.CreateAttribute(psNodeName.Substring(1));
			...
			{
				return pParentNode.AppendChild(pParentNode.OwnerDocument.CreateElement(psNodeName, pNamespaceManager.DefaultNamespace));
			}
		}
		/// <summary>
												...
												bool pbHandleNull)
		{
			...
			if (childNode.NodeType == XmlNodeType.Element)
			{
				...
				{
					childNode.Attributes.RemoveNamedItem(BusinessRuleManagerAttributeName.Null);
				}
				else if (pbHandleNull)
				{
					// Set null attribute
					XmlAttribute nullAttr = pParentNode.OwnerDocument.CreateAttribute(BusinessRuleManagerAttributeName.Null);
		...
		{
			// Find node
			XmlNode childNode = XmlUtil.GetOrCreateNode(pParentNode, psNodeName, pNamespaceManager);
			// Set node value
			childNode.InnerText = psValue;
			// Handle null values
			if (childNode.NodeType == XmlNodeType.Element)
