...
namespace ConsonaCRM.BusinessRuleManager.Grammar.Expressions.String
{
		...
		IGrammarString
	{
		...
		public string EvaluateString(XmlNode pDataNode)
		{
			...
			byte[] value = Convert.FromBase64String(StringContainer.EvaluateString(childNode, this.engine, pDataNode, this.namespaceManager));
			...
			CryptoStream cryptoStream = new CryptoStream(encryptedStream, provider.CreateDecryptor(), CryptoStreamMode.Write);
			...
			// Encode decrypted string as unicode
			return ASCIIEncoding.UTF8.GetString(encryptedStream.ToArray());
		}
		#endregion
	}
}
