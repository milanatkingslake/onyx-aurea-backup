...
if (!(Test-Path "AUT") -or !(Test-Path ".jenkins")) { 
  Write-Host "Run HUTWithCoverage.ps1 from the root of ONYX repository"
  exit 1
}
...
$resultsDir="$onyxRoot\buildlogs\HUTresults"
...
if($run_manual_ut -eq "TRUE") {
  ...
  $outputDir = Get-ChildItem -Recurse -Path Output -Directory
  Write-Host $outputDir
  $testAssemblies = $testAssemblies + (Get-ChildItem -Path $outputDir -Include *.*Tests.dll  -Recurse)
  Write-Host $testAssemblies
}
  
$VStestConsoleArgs="$($testAssemblies) /settings:$onyxRoot\.jenkins\scripts\UTWithCoverage\testsettings.xml /TestAdapterPath:$onyxRoot\AUT\packages\NUnit3TestAdapter.3.12.0\build\net35  /logger:trx;LogFileName=$trxFile /platform:x64 /Parallel /logger:""console;verbosity=minimal"""
